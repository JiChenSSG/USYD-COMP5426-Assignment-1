...
int main(int agrc, char* agrv[]) {
    ...
    double** d;  // 2D matrix, same initial data as a for computation with loop
                 // unrolling
    int n;  // input size
    int n0;
    int i, j, k;
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        for (k = i + 1; k < n; k++) d[k][i] = d[k][i] / d[i][i];
    ...
    if (cnt == 0)
        ...
        printf(
            "Results are incorrect! The number of different elements is %d\n",
            cnt);
}
